...
void Logger::Impl::formatTime() {
    int64_t microSecondSinceEpoch = time_.microSecondSinceEpoch();
    ...
    assert(us.length() == 9);
    stream_ << T(t_time, 17) << T(us.data(), 9);// 这里输出到LogStream对应的缓冲区
} // void formatTime()
...
Logger::~Logger() {
    impl_.finish();
    const logStream::Buffer& buf(stream().buffer());// 把缓冲区取出来放到buf
